...
theOutputDir=file.path(outputDir, "AN_Unadjusted")
theCompareFile=file.path(compareDir, "AN_Unadjusted.tsv")
theRandomSeed=314
#myRandomSeed <- 314
#myTestSeed <- 42
theBatchType="TSS"
...
if (!is.null(inputDir))
{
  ...
  AN_Unadjusted(theBeaData=myData,
              theBatchType=theBatchType,
              thePath=theOutputDir,
              theWriteToFile=TRUE)
  correctedMatrix <- readAsGenericMatrix(file.path(theOutputDir, "ANY_Corrections-ANUnadjusted.tsv"))
