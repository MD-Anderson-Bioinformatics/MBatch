...
theCompareFile=file.path(compareDir, "EB_withNonParametricPriors.tsv")
theRandomSeed=314
#myRandomSeed <- 314
#myTestSeed <- 42
theBatchType="TSS"
...
if (!is.null(inputDir))
{
  ...
  EB_withNonParametricPriors(theBeaData=myData,
                             ...
                             theWriteToFile=TRUE)
  correctedMatrix <- readAsGenericMatrix(file.path(theOutputDir, "ANY_Corrections-EBwithNonParametricPriors.tsv"))
